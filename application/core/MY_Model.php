...
/**
 * @Author            :  Clickr Abin
 * @Create Date       :  2019-07-09 11:46:19
 * @Last Modified by  :  Clickr Abin
 * @Last Modified time:  2019-08-28 16:04:31
 * @email             :  info@clickrweb.com
 * @description       :  總數據模型.
 * @source [http://github.com/jamierumbelow/codeigniter-base-model] 
...
class MY_Model extends CI_Model{
  ...
  /**
   * abin add 20180623
   * Determine if there is a from table alias
   */
  protected $has_from_alias = FALSE;
  /* --------------------------------------------------------------
   * GENERIC METHODS
   * ------------------------------------------------------------ */
  /**
  ...
  public function get_by(){
    ...
    $this->trigger('before_get', $where);
    $this->_set_where($where);
    if ($this->has_from_alias) { #abin add 20180623
  ...
  public function insert($data, $skip_validation = FALSE){
    ...
    if ($data !== FALSE) {
      ...
      $result = $this->_database->insert($this->_table, $data);
      $insert_id = $this->_database->insert_id();
      // $this->trigger('after_create', $insert_id);
      $result = $insert_id > 0 ? $insert_id : $result; #abin change
      $this->trigger('after_create', $result);
      return $insert_id > 0 ? $insert_id : $result; #abin change
    } else {
      return FALSE;
    }
  }
   ...
   */
  public function update($data, $primary_value, $skip_validation = FALSE){
   ...
   */
  public function update_many($data, $primary_values, $skip_validation = FALSE){
    $data = $this->trigger('before_update', $data);
    if ($skip_validation === FALSE) {
      $data = $this->validate($data);
    }
  ...
  public function delete($id){
    ...
    $this->trigger('after_delete', array($id, $result));#abin change 20180623
  ...
  public function delete_many($primary_values){
    ...
    $this->trigger('after_delete', array($primary_values, $result));#abin change 20180623
    // $this->trigger('after_delete', $result);
    return $result;
  }
  /**
  ...
  public function relate($row){
    ...
    foreach ($this->has_many as $key => $value) {
      ...
      if (in_array($relationship, $this->_with)) {
        $this->load->model($options['model'], $relationship . '_model');
        /**abin add**/
        if (isset($options['order_by']) && !empty($options['order_by']) && is_array($options['order_by'])) {
          $this->{$relationship . '_model'}->order_by($options['order_by']);
        }
        /**abin add over**/
        if (is_object($row)) {
  ...
  public function count_all(){
    ...
    if ($this->has_from_alias) { #abin add 20180623
      return $this->_database->count_all();
    } else {
   ...
   */
  public function order_by($criteria, $order = 'ASC', $escape = NULL){
    ...
    if (count($params) == 1 && is_array($params[0])) {
      foreach ($params[0] as $field => $filter) {
        if (is_array($filter)) {
          ...
          if (is_int($field)) {
            $this->_database->where($filter, NULL, FALSE);#abin change
          } else {
   ...
   */
  protected function join($table, $cond, $type = 'LEFT', $escape = NULL){
    $this->_database->join($table, $cond, $type, $escape);
    return $this;
  }
  //abin add 20171023
  public function select_fields($data = '', $escape = NULL){
    if (!empty($data)) {
      $field_string = is_array($data) ? implode(',', $data) : $data;
      $this->_database->select($field_string, $escape);
      return $this;
    }
  }
}